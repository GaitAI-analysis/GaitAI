...
**Hosted provider: the Google Gemini Developer API, on its Free Tier.**
...
Cloudflare Worker (Free)   POST https://ask.gaitai.in/api/ask
    ...
    v
Google Gemini Developer API (Free Tier)   generateContent, GEMINI_API_KEY as a Worker secret
    |
    v
...
network error, malformed JSON, blocked or empty provider output
    |
    v
...
| interim: Hugging Face Inference Providers as the model provider (never deployed) | Google Gemini Developer API, Free Tier |
...
worker/
  ...
  tsconfig.json
  vitest.config.ts         the Cloudflare Vitest plugin; fake key/model bindings; provider mocked
  .dev.vars.example        GEMINI_API_KEY= / GEMINI_MODEL= / ALLOWED_ORIGINS= — names only, no values
  ...
  src/
    ...
    gemini.ts              one generateContent call to the Gemini Developer API, with a deadline
  ...
  test/ask.test.ts         37 tests, provider mocked
```
...
| 503 | `unconfigured` / `model_unconfigured` | GEMINI_API_KEY or GEMINI_MODEL not set |
| 503 | `budget` | the Worker's own daily hosted-call budget is spent |
| 503 | `provider_quota` | Gemini 429 RESOURCE_EXHAUSTED — per-minute rate limit or daily Free Tier quota |
| 502 | `provider_auth` | Gemini 401 UNAUTHENTICATED / 403 PERMISSION_DENIED — key missing, invalid or without permission |
| 502 | `provider_rejected` | Gemini 400 INVALID_ARGUMENT / FAILED_PRECONDITION (e.g. Free Tier not available in the region), 404 model not found |
| 502 | `upstream` | Gemini 5xx, a non-JSON or unexpected reply, a blocked prompt or candidate, or no candidate text |
| 504 | `timeout` | Gemini did not answer within `MODEL_TIMEOUT_MS` |

Never in a response: the API key, the raw Gemini reply, `usageMetadata`,
safety ratings, the system prompt, internal reasoning, environment values, a
stack trace.
...
### The prompt, and how it reaches Gemini

`src/lib/ask/prompt.ts` → `buildMessages()`, called by the Worker and by the
benchmark, so what is benchmarked is what is deployed:

```
system     the Ask GaitAI policy (byte-stable)          → Gemini systemInstruction
history    ≤6 prior turns, text only, roles alternating → Gemini contents, user / model
user       <record> blocks for the canonical records (≤1 500 chars each)
           the page line ("The visitor is currently reading: …")
           the question, labelled, LAST                  → the final user turn
```

`gemini.ts` maps that onto the documented `generateContent` request
(`systemInstruction.parts[].text`, `contents[].role` of `user` / `model`,
`generationConfig`). Sampling is conservative — temperature 0.2, topP 0.9, one
candidate, `maxOutputTokens` from `MODEL_MAX_OUTPUT_TOKENS` (450, hard cap
1 200). No `tools` are sent: **Google Search grounding is deliberately off**,
because GaitAI's canonical records, not the web, define what the assistant may
claim. `thinkingConfig.thinkingLevel` is sent only if `MODEL_THINKING_LEVEL` is
set; thought text is never requested, and any `thought` part that arrives is
discarded.
...
| Provider timeout: `MODEL_TIMEOUT_MS` (22 s) via AbortController, then 504 | yes | `gemini.ts` | Free |
| Runaway output: `MODEL_MAX_OUTPUT_TOKENS` (450), hard cap 1 200 | yes | `env.ts`, `gemini.ts` | Free |
...
The daily budget is **our own** ceiling, independent of Google's quota, and it
is deliberately low (25) while the Free Tier is being evaluated: raise it on
purpose once real usage and quota behaviour are understood. Past it the Worker
answers 503 and every browser falls back to the extractive answer, so the
assistant keeps working. Gemini's own per-minute and per-day Free Tier limits
surface as 429 and are mapped to `provider_quota` with the same fallback.
...
| Name | Kind | Set with |
|---|---|---|
| `GEMINI_API_KEY` | Cloudflare secret | `wrangler secret put GEMINI_API_KEY` (production); `worker/.dev.vars` (local) |
| `GEMINI_MODEL` | non-secret var | `wrangler.jsonc` — **deliberately empty** until the benchmark has run |
| `MODEL_THINKING_LEVEL` | non-secret var | optional `thinkingLevel` (minimal / low / medium / high); empty = model default |
...
The key is never in source, Git, README, `.env.example`, `wrangler.jsonc`, the
browser bundle, a test, or a log line; it travels in the `x-goog-api-key`
header of the one request to `generativelanguage.googleapis.com`, never in a
URL. Wrangler has no schema field to declare a secret as required; the Worker
checks at request time and answers 503 `unconfigured` until it exists. The
model is likewise unset on purpose: while `GEMINI_MODEL` is empty the Worker
answers 503 `model_unconfigured` and the browser falls back. Model selection
happens after the key is configured and `npm run ask:bench` has run.

### Free Tier, plainly

This runs on the Gemini API Free Tier: an API key from Google AI Studio, no
billing account, no credit card, no Vertex AI, no Cloud Run, no paid tools.
Two consequences are documented rather than papered over:

- **Data use.** Google states that content submitted on the Gemini API Free
  Tier may be used to improve its products. Ask GaitAI therefore sends text
  only — the question, the route and title, a short history, and GaitAI's own
  published records — and never sensitive, patient or biometric material.
  Movement Lab stays isolated (§3). No stronger privacy claim is made than
  Google's policy supports.
- **Quota.** The Free Tier is limited per minute and per day. The Worker's own
  budget (25/day by default) is meant to trip first; if Google's limit trips
  instead the visitor still gets the extractive answer.

### Local development and deployment (not yet done)

```bash
cd worker
cp .dev.vars.example .dev.vars      # fill GEMINI_API_KEY and GEMINI_MODEL locally; gitignored
npm run dev                          # wrangler dev, http://localhost:8787/api/ask
npm run typecheck && npm test        # 37 tests, provider mocked
...
To go live, later: `wrangler secret put GEMINI_API_KEY`, set `GEMINI_MODEL`,
uncomment the `routes` block for `ask.gaitai.in`, `wrangler deploy`, then set
...
biometric material — is ever sent to the Worker or to Gemini. There is no
field for it, the validator drops unknown keys, `gemini.ts` builds text parts
only, and Movement Lab does not import the assistant. Movement Lab keeps its
own architecture.
...
| Web grounding | off: no `tools` are sent to Gemini; the canonical records are the only evidence |
...
| Key exposure | a Worker secret binding, read into one request header |
...
npm run worker:test           # the Worker's 37 tests, Gemini mocked (CI)
npm run worker:check          # wrangler deploy --dry-run
GEMINI_API_KEY=… npm run ask:bench   # score Free Tier candidates on the brief's questions + the 25
```

The Worker suite covers: OPTIONS from accepted and rejected origins; POST from
a rejected origin; no Origin; GET; unknown paths; invalid JSON; oversized body,
question, history and record list; unknown record ids dropped; no canonical
records; missing key; missing model; Gemini 401/403, 429, 400/404, 5xx; a
non-JSON reply; no candidate text; a blocked prompt and a blocked candidate
without exposing safety internals; thought parts and reasoning traces never
returned; bare-URL and off-allowlist-link sanitisation; a model-authored
Sources block; a successful grounded answer with deterministic canonical
sources; the key reaching only the Gemini request header and never a response
or URL; browser-supplied "evidence" and non-text fields never reaching the
prompt; history mapped to alternating user/model turns; per-caller burst; the
hourly limit; the daily budget; the conservative default budget; the timeout;
...
provider is the Google Gemini Developer API, on its Free Tier, and Google
states that Free Tier content may be used to improve its products. That is why
Ask GaitAI is textual, why patient and biometric material is never sent, and
why Movement Lab is isolated. The Worker logs structured metadata only (status,
record count, token counts, finish reason, failure class), never the question
or the answer; stores per-caller timestamps under a daily-rotating salted hash
for at most an hour of activity; and keeps no transcript. The browser sends no
...
```bash
GEMINI_API_KEY=… npm run ask:bench
GEMINI_API_KEY=… npm run ask:bench -- --models gemini-3.8-flash,gemini-2.5-flash
GEMINI_API_KEY=… npm run ask:bench -- --brief --json tmp/bench.json
GEMINI_API_KEY=… npm run ask:bench -- --thinking low
```

Scores grounding, hallucinated figures, boundary breaches, invented module
names, instruction following, source support, latency and failures for each
candidate, through the same `buildMessages()` and the same `generate()` the
Worker uses. Calls are paced for the Free Tier's per-minute limits and a 429
stops that model's run rather than retrying. Cost is reported as "Free Tier".

The comparison set is deliberately four models, not every one the Free Tier
lists, so a full run does not burn the daily quota. All four show Free Tier
input/output on the Gemini API pricing page (refreshed 2026-09-05):
...
No live run has been made yet — `GEMINI_API_KEY` has not been supplied — so
there is no leader. Record the winner and its numbers here, then set
`GEMINI_MODEL`.
