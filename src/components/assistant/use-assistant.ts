...
export function useAssistant(page: PageContext): AssistantState {
  ...
  useEffect(() => {
    ...
    /* Learn the CURRENT hosted configuration before the first question, so a
       ...
       an endpoint it was never told about. The thread stays keyed to the
       BUNDLE that wrote it (see readStored): a new deployment loads a new
       bundle and drops the old thread; a stale bundle keeps its own thread and
       simply talks to the current endpoint. Re-keying to the server's build
       here would race a question asked in the first few hundred milliseconds
       and wipe it mid-flight — it did, once. */
