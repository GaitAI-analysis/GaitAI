...
function message(
  state: State,
): { text: string; detail?: string; tone: "ok" | "warn" | "bad" } | null {
  switch (state) {
    case "success":
      /* The trajectory has completed: the verdict is the arrival, the detail
         says what happens next. Both live in the one status region so a
         screen reader hears them together, once. */
      return {
        text: "You're on the signal.",
        detail: "We'll send the next GaitAI update here.",
        tone: "ok",
      };
    case "already":
...
export function SubscribeForm({
  ...
  return (
    <section
      ...
      /* The signal's three states, read by the stylesheet: the field holds a
         well-formed address (the lane brightens and the node arms), or the
         address is stored (the lane completes and the node fills). Focus is
         the third and comes from :focus-within. Decorative only — the text
         in the status region is what says so. */
      data-valid={!done && isValidEmail(email) ? "true" : undefined}
    ...
    >
      ...
      {/* THE FIELD GROUP: the form, the support line, the verdict and the small
          print, kept together so their spacing is their own. It is a real box
          only in the blog block's desktop composition (where the lane is drawn
          from its left edge); elsewhere it is `display: contents`. */}
      <div className={styles.field}>
        ...
        >
          {note?.text ?? ""}
          {note?.detail && <span className={styles.noteDetail}>{note.detail}</span>}
        </p>
