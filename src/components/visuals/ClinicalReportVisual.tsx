...
/**
 * A neutral clinical-report schema preview. It intentionally avoids patient
 * identifiers, values and outcomes until a real analysis record is supplied.
 */
export function ClinicalReportVisual() {
  const gradientId = `report-gradient-${useId().replace(/:/g, "")}`;
  return (
    <div
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <ScoreCard label="Mobility score" value="—" />
        <ScoreCard label="Risk signal" value="—" />
      </div>

      {/* Metrics row */}
      <div className="mt-3 grid grid-cols-4 gap-2">
        <Metric label="Speed" value="—" unit="" />
        <Metric label="Cadence" value="—" unit="" />
        <Metric label="Stride" value="—" unit="" />
        <Metric label="Asym." value="—" unit="" />
      </div>
      ...
      <div className="mt-4 rounded-xl border border-white/8 bg-white/[0.02] p-3">
        <div className="flex items-center justify-between">
          <div className="text-[10px] font-medium uppercase tracking-[0.18em] text-soft-mute">
            Longitudinal movement trace
          </div>
          <div className="text-[10px] font-semibold text-soft-mute">
      ...
      <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.03] py-2.5 text-center text-xs font-medium text-soft-mute">
        Report fields populate only when an analysis record is supplied
      </div>
    </div>
  );
}
